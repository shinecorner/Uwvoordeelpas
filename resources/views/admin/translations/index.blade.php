...
<script type="text/javascript">
    $(document).ready(function(){
        ...
        $('.group-select').on('change', function(){
            var group = $('.ui.normal.dropdown.group-select').dropdown('get value');

            if (group) {
                window.location.href = '{{ action('Admin\TranslationController@getView') }}/' + $('.ui.normal.dropdown.group-select').dropdown('get value');
            } else {
                window.location.href = '{{ action('Admin\TranslationController@getIndex') }}';
            }
        });
...
<div class="content">
    ...
    <div class="ui message icon success success-import" style="display:none;">
        <i class="thumbs up icon"></i>
        <p>Er zijn <strong class="counter">N</strong> vertalingen succesvol geimporteerd.</p>
    </div>

    <div class="ui message success success-find" style="display:none;">
        <p>Done searching for translations, found <strong class="counter">N</strong> items!</p>
    </div>

    <div class="ui message success icon success-publish" style="display:none;">
        <i class="thumbs up icon"></i>
        U heeft succesvol de vertalingen van groep <strong> '{{ $group }}' </strong> gepubliceerd!
    </div>
    ...
    <div class="ui grid">
        <div class="nine wide column">
            @if(!isset($group))
            <form class="form-inline form-import" method="POST" action="{{ action('Admin\TranslationController@postImport') }}" data-remote="true" role="form">
                <input type="hidden" name="_token" value="<?php echo csrf_token(); ?>">

                <select name="replace" class="ui normal dropdown form-control">
                    <option value="0">Voeg nieuwe vertalingen toe</option>
                    <option value="1">Vervang bestaande vertalingen</option>
                </select>

                <button type="submit" class="ui green button"  data-disable-with="Laden..">Importeer groepen</button>
            </form>
            @endif

            @if (isset($group))
                <form class="form-inline form-publish" method="POST" action="{{ action('Admin\TranslationController@postPublish', $group) }}" data-remote="true" role="form" data-confirm="Weet je zeker dat je deze groep '<?= $group ?> wil publiceren? Dit zal bestaande vertalingen overschrijven.">
                    <input type="hidden" name="_token" value="<?php echo csrf_token(); ?>">

                    <button type="submit" class="ui green icon left labeled button" data-disable-with="Publiceren..">
                        <i class="save icon"></i>
                        Publiceer
                    </button>

                    <a href="{{ action('Admin\TranslationController@getIndex') }}" class="ui icon left labeled button">
        ...
        <div class="five wide column">
            <form role="form">
                ...
                <div class="form-group">
                    <select name="group" id="group" class="group-select ui normal dropdown">
    ...
    @if ($group)
    <h5 class="ui header right floated">Totaal <span class="ui label">{{ $numTranslations }}</span> Aangepast <span class="ui label green">{{ $numChanged }}</span></h5>

    <table class="ui very basic collapsing celled table list" style="width: 100%;">
        <thead>
            <tr>
                <th width="15%">Sleutelwoord</th>
                @foreach($locales as $locale)
                    <th><i class="{{ $locale == 'en' ? 'gb' : $locale }} flag"></i> {{ strtoupper($locale) }}</th>
                @endforeach
    ...
    <form class="ui form" action="{{ action('\Barryvdh\TranslationManager\Controller@postAdd', array($group)) }}" method="POST" role="form">
        <input type="hidden" name="_token" value="<?php echo csrf_token(); ?>">
        
        <textarea class="form-control" rows="3" name="keys" placeholder="Voeg 1 sleutelwoord per regel toe"></textarea><br /><br />
        <button type="submit" value="Toevoegen" class="ui button">Toevoegen</button>
    </form>
    @endif
</div>
@stop